...
public class Point3D {
	...
	Point3D cross(Point3D a) {
		return new Point3D(y * a.z - z * a.y, z * a.x - x * a.z, x * a.y - y * a.x);
	}
	...
	Point3D rotX(float ang) {
		...
		float[][] rotMat = new float[][] { { 1, 0, 0 }, { 0, cos, -sin }, { 0, sin, cos } };
	...
	Point3D rotY(float ang) {
		float cos = (float) Math.cos(ang);
		float sin = (float) Math.sin(ang);
		float[][] rotMat = new float[][] { { cos, 0, sin }, { 0, 1, 0 }, { -sin, 0, cos } };
		return rotPoint(rotMat);
	}
	...
	Point3D rotZ(float ang) {
		float cos = (float) Math.cos(ang);
		float sin = (float) Math.sin(ang);
		float[][] rotMat = new float[][] { { cos, -sin, 0 }, { sin, cos, 0 }, { 0, 0, 1 } };
		return rotPoint(rotMat);
	}
